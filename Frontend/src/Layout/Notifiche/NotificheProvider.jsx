...
export const NotificheProvider = ({ children }) => {
  ...
  const clearNotificationForUser = async ({ type, fromUserId, postId }) => {
    ...
    try {
      await fetch("https://apuliaevents.onrender.com/api/notifiche/segnaComeVista", {
        method: "PUT",
 ...
 useEffect(() => {
  fetch("https://apuliaevents.onrender.com/api/user/me", { credentials: "include" })
    .then((r) => r.json())
    .then((u) => {
      if (u && u._id) {
        setUserId(u._id);
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch("https://apuliaevents.onrender.com/api/notifiche/notifiche", {
      credentials: "include",
    })
      .then((r) => r.json())
      .then((offline) => {
  ...
  useEffect(() => {
    ...
    const onNewMessage = (msg) => {
      ...
      if (chatAperta === msg.IdMittente) {
        ...
        fetch(
          `https://apuliaevents.onrender.com/api/messaggi/SegnaComeLetto/${msg.IdMittente}`,
          {
            method: "PUT",
            credentials: "include",
          }
