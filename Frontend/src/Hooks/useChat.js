...
const SERVER_URL = "https://apuliaevents.onrender.com";
...
export function useChat(selectedUserId, selectedUsername, setNuovoMessaggio) {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            const messaggioSalvato = await res.json();
            setMessages((prev) => [...prev, messaggioSalvato]);
            setTimeout(() => {
                if (setNuovoMessaggio) {
                    setNuovoMessaggio(messaggioSalvato);
                        }
            }, 100);
        } catch (err) {
