...
import Notification from "../Models/Notifiche_model.js";
...
export const CreaPost = async (req, res) => {
  try {
    ...
    if (!oraEvento || oraEvento.trim() === "")
      return res.status(400).json({ message: "L'orario dell'evento è obbligatorio" });

    if (!luogo || luogo.trim() === "")
      return res.status(400).json({ message: "Il luogo (indirizzo) è obbligatorio" });
